...
- [x] 4.1 Create DTO records `BulkAddCensoRequest.java`, `BulkAddCensoResponse.java`, and `CensoEntryResponse.java` in `co/com/votapp/ws/electoral/application/dto`.
- [x] 4.2 Create `CensoController.java` in `co/com/votapp/ws/electoral/infrastructure/adapter/in/web` with endpoints for bulk add, individual add/remove, clear, and list.
- [x] 4.3 Write unit tests in `co/com/votapp/ws/electoral/infrastructure/adapter/in/web/CensoControllerTest.java` asserting HTTP 200/201/204 codes, exception propagation, and proper delegation to the use case.
- [x] 4.4 (bonus) Add `ElectionNotModifiableException` → HTTP 409 handler to `GlobalExceptionHandler.java`.
