...
- Al activar una elección, el sistema emite **automáticamente** un token de votación para cada funcionario habilitado (bulk issuance).
- Para cada token se genera un **tokenHash** SHA-256 que se persiste en la base de datos, y un **tokenId** UUID para referencias internas.
...
- El token atraviesa los estados: `ISSUED` → `USED` (cuando se vota).

**Modelo de token**:

```
tokenHash (SHA-256, se persiste en la DB)
    │
    ▼ UUID interno
tokenId (identificador para referencias internas — resuelto por sesión portal)
```

**Valor para la organización**:

- **Seguridad**: la credencial del funcionario es su sesión en el portal. El tokenId se resuelve internamente; el funcionario nunca ve ni maneja un token en texto plano.
- **Un voto por persona**: el índice único parcial impide emitir dos tokens activos al mismo funcionario en la misma elección.
- **Anonimato**: durante la votación, el sistema identifica al funcionario por sesión para resolver el tokenId; la tabla `votos` no almacena `funcionario_id`.
...
**Lo que hace el sistema** — el flujo de portal (`CastVoteByTokenId`):

1. El funcionario se autentica en el **portal** con sus credenciales (usuario/contraseña).
2. La sesión activa del portal permite al sistema resolver el **tokenId** del funcionario para esa elección, sin que el funcionario vea ni maneje un token en texto plano.
3. El funcionario elige un **candidato** en la pantalla de votación.
4. El sistema adquiere un **bloqueo atómico en Redis** mediante `SETNX` (Set if Not eXists) para evitar que dos solicitudes concurrentes usen el mismo token.
5. Si el bloqueo falla → el token ya está siendo usado → se responde con `409 Conflict`.
6. Si el bloqueo se adquiere → se ejecuta una **transacción en PostgreSQL** que:
   - Revalida el token por su ID (existe, está en estado ISSUED).
...
7. Se libera el bloqueo en Redis.
...
```
┌──────────────────────────────────────────────────┐
│            PORTAL CASTVOTE (por tokenId)          │
│                                                   │
│   sesión_portal → tokenId + candidateId           │
│       │                                           │
│       ▼                                           │
│   ┌────────────────┐                              │
...
- **Atomicidad real**: no hay condición de carrera. Si 100 solicitudes usan el mismo tokenId simultáneamente, solo un voto se registra.
...
2. **Los tokens se emiten en bloque al activar la elección**: no hay emisión manual de tokens uno a uno. El proceso es automático, transaccional e idempotente.
3. **El funcionario nunca ve ni maneja un token en texto plano**: la sesión de portal resuelve el tokenId internamente. El flujo de rawToken manual ha sido eliminado.
4. **La atomicidad se logra con Redis + PostgreSQL**: Redis evita la contención (rápido), PostgreSQL garantiza la consistencia (confiable).
5. **Las reglas de negocio viven en el dominio Java, no en la base de datos**: no hay funciones PL/pgSQL con lógica electoral. El código es testeable, versionable y revisable.
